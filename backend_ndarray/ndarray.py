...
class NDArray:
    ...
    # 不拷贝底层数据，重新设置shape与strides，并返回新NDArray
    ...
    def reshape(self, new_shape):
        ...
        # as_strided()原地修改shape和strides，compact_strides()根据shape计算strides
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = [x for x in range(len(self.shape))]
        new_strides = [x for x in range(len(self.strides))]
        for i in range(len(self.shape)):
            new_shape[i] = self.shape[new_axes[i]]
        for i in range(len(self.shape)):
            new_strides[i] = self.strides[new_axes[i]]
        return self.as_strided(tuple(new_shape), tuple(new_strides))
        ### END YOUR SOLUTION

    def broadcast_to(self, new_shape: tuple):
        """
        ...
        ### BEGIN YOUR SOLUTION
        # 将Array的纬度数量扩展至与new_shape的纬度数量相同
        shape_expand = [1] * len(new_shape)
        shape_expand[len(new_shape) - len(self.shape):] = self.shape
        # 判断被广播的维度的长度是否为1，等价于判断原来的shape中是否存在长度不为1，且与对应新的shape不相等的纬度
        # new_shape[i] != shape[i] for all i where shape[i] != 1
        for dim, new_dim in zip(shape_expand, new_shape):
            if dim != 1 and dim != new_dim:
                raise ValueError('the shape can not be broadcasted to new shape')
        # 对于维度长度为1的纬度，计算其合法的数组索引时，进行计算的下标均为0
        # e.g：A 3*1 strides = [1, 1]，计算索引：i * 1 + j * 1，合法的下标j恒等于0
        # 因此将原来的shape中，值为1对应的stride赋值为0
        new_strides = [0] * len(new_shape)
        new_strides[len(new_shape) - len(self.shape):] = self.strides
        for i, stride in enumerate(self.shape):
            if stride == 1:
                new_strides[i] = 0
        return self.as_strided(tuple(new_shape), tuple(new_strides))
        ### END YOUR SOLUTION
    ...
    # 工具函数，处理__getitem__()传递的参数
    # sl为切片元组中的一个切片，dim为维度的下标
    ...
    def __getitem__(self, idxs):
        ...
        for i, idx in enumerate(idxs):
            # shape的计算类似cuda计算需要的block数量
            new_shape.append((idx.stop - idx.start + idx.step - 1) // idx.step)
            # offset为底层真实0号元素到视图0号元素的偏移量
            offset += idx.start * self.strides[i]
            # 视图的底层数据非连续，不能使用compact_strides()计算strides
            # 当step不为1时，某维度的stride应变为step倍数
